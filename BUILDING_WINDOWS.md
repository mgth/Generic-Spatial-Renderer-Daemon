# Building gsrd with SPARTA on Windows

This guide documents how to build gsrd with the `sparta` feature on Windows, enabling runtime VBAP gain table generation via the `generate-vbap` command.
...
The SPARTA feature depends on two C libraries:
...
# SPARTA + ASIO + Windows Service (full Windows build)
cargo build --release --features sparta,asio

# SPARTA only (no ASIO, no Windows Service)
...
|---|---|
| `sparta` | default render flow, `generate-vbap` |
| `asio` | default render flow, `list-asio-devices` (`--output-backend asio`) |
| `sparta,asio` | All of the above |
...
```bash
export SAF_ROOT="C:/dev/SAF"
cargo build --features sparta,asio --release
```
