...
//!
//! Speaker position updates (via `/gsrd/control/speaker/{idx}/{az|el|distance}` +
//! `/gsrd/control/speakers/apply`) trigger a background recompute of the VBAP
...
pub struct RendererControl {
    ...
    /// Reset to `false` by a successful `/gsrd/control/save_config`.
