...
impl SpartaVbapLayout {
    ...
    // We keep gsrd's public spread model normalized in [0, 1] and map it here.
...
impl VbapPanner {
    ...
    #[inline]
    fn table_z_value(&self, zi: usize, z_size: usize) -> f32 {
        if self.allow_negative_z {
            -1.0 + 2.0 * (zi as f32) / ((z_size - 1) as f32)
        } else {
    ...
    ///
    /// **Note:** This method requires the `sparta` feature to be enabled.
        ...
        match table_mode {
            ...
            VbapTableMode::Cartesian {
                ...
                if x_size < 2 || y_size < 2 || z_size < 2 {
                    return Err("Cartesian table sizes must be >= 2 for each axis".to_string());
                }
                self.cartesian_cache = Some(self.build_cartesian_cache(x_size, y_size, z_size)?);
                log::info!(
    ...
    fn build_cartesian_cache(
        ...
        let grid_points = x_size * y_size * z_size;
        ...
        for table_idx in 0..self.spread_tables.len() {
            ...
            for zi in 0..z_size {
                let z = self.table_z_value(zi, z_size);
                for yi in 0..y_size {
    ...
    /// # Behavior
    ///
    /// - Direct `sparta` mode uses `vbap3D` with continuous spread
    /// - Non-sparta mode interpolates from preloaded tables
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    fn get_gains_from_cartesian_cache(&self, x: f32, y: f32, z: f32) -> Gains {
        ...
        let fz = if self.allow_negative_z {
            (z + 1.0) * 0.5 * (cache.z_size - 1) as f32
        } else {
