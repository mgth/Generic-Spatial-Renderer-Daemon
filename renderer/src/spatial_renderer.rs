...
//!
//! ```no_run
//! use gsrd::spatial_renderer::SpatialRenderer;
//! use gsrd::speaker_layout::SpeakerLayout;
//! use gsrd::spatial_vbap::{DistanceModel, VbapTableMode};
//!
//! // Load speaker layout
...
//!     gsrd::live_params::VbapBackendMode::Polar,      // bridge preferred mode
//!     gsrd::live_params::LiveVbapTableMode::Polar,    // initial live selection
...
struct ChannelState {
    ...
    ramp_length: u64,
    /// Ramp duration in frames
    remaining_ramp_frames: Option<u64>,
...
impl RampProcessor for ChannelState {
    fn process_ramp(&mut self, step: usize) -> RampStatus {
        if let Some(remaining_frames) = self.remaining_ramp_frames {
            let step_u64 = step as u64;

            if remaining_frames > step_u64 {
                let fraction = step_u64 as f64 / remaining_frames as f64;

                // Interpolate current state towards target
                self.current = self.current.interpolate(&self.target, fraction);

                self.remaining_ramp_frames = Some(remaining_frames - step_u64);
                RampStatus::Ramping
            } else {
                self.current = self.target;
                self.remaining_ramp_frames = None;
                RampStatus::Finished
            }
        } else {
            RampStatus::Idle
...
impl SpatialRenderer {
    ...
    ///
    /// **Note:** This method requires the `sparta` feature to generate VBAP tables.
    /// Without sparta, use `from_vbap()` to load pre-generated tables.
    #[cfg(feature = "sparta")]
    pub fn new(
            ...
            if let Some(target_position) = event.position {
                if state.target.position != target_position || spread_changed {
                    if self.log_object_positions {
                        let remaining_frames = state.remaining_ramp_frames.unwrap_or(0);
                        let sample_pos = event.sample_pos.unwrap_or(0);
                        if state.target.position != target_position {
                            log::info!(
                                "  Obj ch{:2}: sample_pos {} remaining {} - Starting ramp over {} frames (~{}ms)",
                                event.channel_idx,
                                sample_pos,
                                remaining_frames,
                                state.ramp_length,
                    ...
                    state.remaining_ramp_frames = Some(state.ramp_length);
                    state.target_sample_index = event.sample_pos;
                }
            } else if spread_changed {
                if state.remaining_ramp_frames.is_none() {
        ...
        for input_channel_idx in 0..input_channel_count {
            ...
            if input_channel_idx < num_beds {
                ...
                for sample_idx in 0..sample_length {
                    ...
                    let scaled_z = rendering_position[2] as f32 * live.room_ratio[2];
