...
///
/// - Linux:   `$XDG_CONFIG_HOME/gsrd/config.yaml`  (fallback: `~/.config/gsrd/config.yaml`)
...
pub fn default_config_path() -> Option<PathBuf> {
    #[cfg(windows)]
    {
        return std::env::var("APPDATA")
            .ok()
            .map(|p| PathBuf::from(p).join("gsrd").join("config.yaml"));
    }
    ...
    Some(base.join("gsrd").join("config.yaml"))
}
