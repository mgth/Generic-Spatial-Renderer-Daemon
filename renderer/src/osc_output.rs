...
pub struct OscSender {
    ...
    /// `Some(t)`    = registered via `/gsrd/register`, must send `/gsrd/heartbeat`
    ///                every <CLIENT_TIMEOUT/2 seconds or it will be dropped.
    clients: Arc<Mutex<HashMap<SocketAddr, Option<Instant>>>>,
...
impl OscSender {
    pub fn new(default_target: SocketAddrV4) -> Result<Self> {
        ...
        clients.insert(SocketAddr::V4(default_target), None);
        Ok(Self {
    ...
    ///
    /// Clients send `/gsrd/register [i listen_port?]` from their listening socket.
                        ...
                        Ok((len, src)) => {
                            match rosc::decoder::decode_udp(&buf[..len]) {
                                Ok((_, OscPacket::Message(msg)))
                                    if msg.addr == "/gsrd/register" =>
                                {
                                    let client = resolve_register_addr(src, &msg.args);
                                    let prev = clients
                                        .lock()
                                        .unwrap()
                                        .insert(client, Some(Instant::now()));
                                    if prev.is_none() {
                                ...
                                Ok((_, OscPacket::Message(msg)))
                                    if msg.addr.starts_with("/gsrd/control/") =>
                                {
                                    if let Some(ref ctrl) = control {
    ...
    fn send_to_all(&self, bytes: &[u8]) {
        ...
        // Prune dead clients (those that missed heartbeats).
        ...
        for addr in clients.keys() {
            if let Err(e) = self.socket.send_to(bytes, addr) {
                log::warn!("OSC send error to {}: {}", addr, e);
            }
        }
    }
    ...
    ///
    /// - `/gsrd/state/loudness/source i <dBFS>` - source loudness metadata value from stream
    /// - `/gsrd/state/loudness/gain f <linear>` - correction gain as linear ratio (1.0 if disabled)
    pub fn send_loudness_state(&self) {
        ...
        if let Some(dl) = live.dialogue_level {
            broadcast_int(socket, clients, "/gsrd/state/loudness/source", dl as i32);
        }
        ...
        broadcast_float(socket, clients, "/gsrd/state/loudness/gain", gain_linear);
    }
    ...
    /// Format: /gsrd/bed/config channel_count
    pub fn send_bed_config(&self, channel_count: u32) -> Result<()> {
        let msg = OscMessage {
            addr: "/gsrd/bed/config".to_string(),
    ...
    /// Format: /gsrd/timestamp samples seconds
    pub fn send_timestamp(&self, sample_pos: u64, seconds: f64) -> Result<()> {
        let msg = OscMessage {
            addr: "/gsrd/timestamp".to_string(),
    ...
    ///
    /// Always emits `/gsrd/spatial/frame` with the total object count.
    /// Per-object `/gsrd/object/{id}/xyz` messages are only sent for objects
    ...
    pub fn send_object_frame(
        ...
        let frame_msg = OscMessage {
            addr: "/gsrd/spatial/frame".to_string(),
            args: vec![
        ...
        for stale_id in objects.len()..prev_len {
            let msg = OscMessage {
                addr: format!("/gsrd/object/{}/xyz", stale_id),
                args: vec![
                    OscType::Float(0.0),
                    OscType::Float(0.0),
                    OscType::Float(0.0),
        ...
        for (object_id, obj) in objects.iter().enumerate() {
            ...
            if changed {
                let msg = OscMessage {
                    addr: format!("/gsrd/object/{}/xyz", object_id),
                    args: vec![
                        OscType::Float(obj.x),
                        OscType::Float(obj.y),
                        OscType::Float(obj.z),
    ...
    ///   `/gsrd/meter/object/{idx}`        [f peak_dBFS, f rms_dBFS]
    ///   `/gsrd/meter/object/{idx}/gains`  [f g0, f g1, ..., f gN]
    pub fn send_meter_bundle(
        ...
        if let Some(ms) = latency_target_ms.or(latency_instant_ms) {
            messages.push(OscPacket::Message(OscMessage {
                addr: "/gsrd/state/latency".to_string(),
                args: vec![OscType::Float(ms)],
            }));
        }
        if let Some(ms) = latency_instant_ms {
            messages.push(OscPacket::Message(OscMessage {
                addr: "/gsrd/state/latency_instant".to_string(),
                args: vec![OscType::Float(ms)],
            }));
        }
        ...
        if let Some(ref control) = self.control {
            ...
            if let Some(rate) = rate_opt {
                messages.push(OscPacket::Message(OscMessage {
                    addr: "/gsrd/state/audio/sample_rate".to_string(),
                    args: vec![OscType::Int(rate as i32)],
                }));
            }
            if !fmt.is_empty() {
                messages.push(OscPacket::Message(OscMessage {
                    addr: "/gsrd/state/audio/sample_format".to_string(),
        ...
        for (idx, &(peak, rms)) in snapshot.speaker_levels.iter().enumerate() {
            messages.push(OscPacket::Message(OscMessage {
                addr: format!("/gsrd/meter/speaker/{}", idx),
                args: vec![OscType::Float(peak), OscType::Float(rms)],
            }));
        }
...
///
/// Format:
///   `/gsrd/config/speakers`    [i count]
///   `/gsrd/config/speaker/{i}` [s name, f azimuth_deg, f elevation_deg, f distance_m, i spatialize, f delay_ms]
...
fn broadcast_speaker_config(
    socket: &UdpSocket,
    clients: &Mutex<HashMap<SocketAddr, Option<Instant>>>,
    layout: &crate::speaker_layout::SpeakerLayout,
) {
...
/// Dispatch a `/gsrd/control/…` message.
fn handle_control_message(
    ...
    // ── Save config ──────────────────────────────────────────────────────────
    if addr == "/gsrd/control/save_config" {
        save_live_config(control, socket, clients);
        return;
    }

    // ── Output sample-rate control ───────────────────────────────────────────
    if addr == "/gsrd/control/audio/sample_rate" {
    ...
    if addr == "/gsrd/control/adaptive_resampling" {
    ...
    if addr == "/gsrd/control/latency_target" {
        ...
        if let Some(ms) = latency_ms {
            ...
            set_dirty(control, socket, clients);
            broadcast_float(socket, clients, "/gsrd/state/latency_target", ms as f32);
        }
        return;
    }

    // ── Export current layout to its own YAML file ──────────────────────────
    if addr == "/gsrd/control/layout/export" {
    ...
    // ── Layout radius (meters per unit): /gsrd/control/layout/radius_m f ───
    if addr == "/gsrd/control/layout/radius_m" {
        if let Some(OscType::Float(f)) = msg.args.first() {
            let v = f.max(0.01);
            control.with_editable_layout(|layout| layout.radius_m = v);
            set_dirty(control, socket, clients);
            broadcast_float(socket, clients, "/gsrd/state/layout/radius_m", v);
    ...
    // ── Master gain ─────────────────────────────────────────────────────────
    if addr == "/gsrd/control/gain" {
        if let Some(OscType::Float(f)) = msg.args.first() {
            let gain = *f;
            control.live.write().unwrap().master_gain = gain;
            set_dirty(control, socket, clients);
            broadcast_float(socket, clients, "/gsrd/state/gain", gain);
        }
        return;
    }

    // ── Spread min ──────────────────────────────────────────────────────────
    if addr == "/gsrd/control/spread/min" {
        if let Some(OscType::Float(f)) = msg.args.first() {
            control.live.write().unwrap().spread_min = *f;
            set_dirty(control, socket, clients);
            broadcast_float(socket, clients, "/gsrd/state/spread/min", *f);
    ...
    // ── Spread max ──────────────────────────────────────────────────────────
    if addr == "/gsrd/control/spread/max" {
        if let Some(OscType::Float(f)) = msg.args.first() {
            control.live.write().unwrap().spread_max = *f;
            set_dirty(control, socket, clients);
            broadcast_float(socket, clients, "/gsrd/state/spread/max", *f);
            trigger_layout_recompute(control, socket, clients);
        }
        return;
    }
    ...
    if addr == "/gsrd/control/spread/from_distance" {
    ...
    if addr == "/gsrd/control/spread/distance_range" {
        if let Some(OscType::Float(f)) = msg.args.first() {
            control.live.write().unwrap().spread_distance_range = *f;
            set_dirty(control, socket, clients);
            broadcast_float(socket, clients, "/gsrd/state/spread/distance_range", *f);
            trigger_layout_recompute(control, socket, clients);
        }
        return;
    }
    ...
    if addr == "/gsrd/control/spread/distance_curve" {
        if let Some(OscType::Float(f)) = msg.args.first() {
            control.live.write().unwrap().spread_distance_curve = *f;
            set_dirty(control, socket, clients);
            broadcast_float(socket, clients, "/gsrd/state/spread/distance_curve", *f);
            trigger_layout_recompute(control, socket, clients);
        }
        return;
    }

    // ── Cartesian VBAP table sizes: /gsrd/control/vbap/cart/{x_size|y_size|z_size} ──
    if let Some(rest) = addr.strip_prefix("/gsrd/control/vbap/cart/") {
        ...
        if let Some(size) = size {
            let state_addr = match rest {
                "x_size" => {
                    ...
                    Some("/gsrd/state/vbap/cart/x_size")
                }
                "y_size" => {
                    control.live.write().unwrap().vbap_cart_y_size = size;
                    Some("/gsrd/state/vbap/cart/y_size")
                }
                "z_size" => {
                    control.live.write().unwrap().vbap_cart_z_size = size;
                    Some("/gsrd/state/vbap/cart/z_size")
                }
                _ => None,
            };
    ...
    // ── VBAP table mode: /gsrd/control/vbap/table_mode [s auto|polar|cartesian] ──
    if addr == "/gsrd/control/vbap/table_mode" {
    ...
    //    /gsrd/control/vbap/polar/{azimuth_resolution|elevation_resolution|distance_res|distance_max} ──
    if let Some(rest) = addr.strip_prefix("/gsrd/control/vbap/polar/") {
        match rest {
            ...
            "distance_res" => {
                ...
                if let Some(res) = res {
                    ...
                    set_dirty(control, socket, clients);
                    broadcast_int(socket, clients, "/gsrd/state/vbap/polar/distance_res", res);
                    trigger_layout_recompute(control, socket, clients);
                }
            }
            "distance_max" => {
    ...
    // ── Room ratio: /gsrd/control/room_ratio w l h ────────────────────────
    if addr == "/gsrd/control/room_ratio" {
        if msg.args.len() >= 3 {
            ...
            if let (Some(w), Some(l), Some(h)) = (w, l, h) {
                ...
                set_dirty(control, socket, clients);
                broadcast_fff(socket, clients, "/gsrd/state/room_ratio", w, l, h);
    ...
    // ── Room rear ratio: /gsrd/control/room_ratio_rear r ───────────────────
    if addr == "/gsrd/control/room_ratio_rear" {
        if let Some(OscType::Float(f)) = msg.args.first() {
            let v = f.max(0.01);
            control.live.write().unwrap().room_ratio_rear = v;
            set_dirty(control, socket, clients);
            broadcast_float(socket, clients, "/gsrd/state/room_ratio_rear", v);
    ...
    // ── Room center blend: /gsrd/control/room_ratio_center_blend b ─────────
    if addr == "/gsrd/control/room_ratio_center_blend" {
        if let Some(OscType::Float(f)) = msg.args.first() {
            let v = f.clamp(0.0, 1.0);
            control.live.write().unwrap().room_ratio_center_blend = v;
            set_dirty(control, socket, clients);
            broadcast_float(socket, clients, "/gsrd/state/room_ratio_center_blend", v);
    ...
    // ── Distance diffuse blend: /gsrd/control/distance_diffuse/{enabled|threshold|curve} ──
    if let Some(rest) = addr.strip_prefix("/gsrd/control/distance_diffuse/") {
        match rest {
            "enabled" => {
                if let Some(OscType::Int(i)) = msg.args.first() {
                    let v = *i != 0;
                    ...
                    broadcast_int(
                        socket,
                        clients,
                        "/gsrd/state/distance_diffuse/enabled",
                        if v { 1 } else { 0 },
                    );
                    trigger_layout_recompute(control, socket, clients);
                }
            ...
            "threshold" => {
                if let Some(OscType::Float(f)) = msg.args.first() {
                    ...
                    set_dirty(control, socket, clients);
                    broadcast_float(socket, clients, "/gsrd/state/distance_diffuse/threshold", v);
                    trigger_layout_recompute(control, socket, clients);
                }
                return;
            }
            "curve" => {
                if let Some(OscType::Float(f)) = msg.args.first() {
                    ...
                    set_dirty(control, socket, clients);
                    broadcast_float(socket, clients, "/gsrd/state/distance_diffuse/curve", v);
                    trigger_layout_recompute(control, socket, clients);
                }
                return;
            }
    ...
    // ── Per-object gain / mute: /gsrd/control/object/{idx}/{gain|mute} ───
    if let Some(rest) = addr.strip_prefix("/gsrd/control/object/") {
            ...
            if let Ok(idx) = idx_str.parse::<usize>() {
                if let Some(OscType::Float(f)) = msg.args.first() {
                    ...
                    broadcast_float(
                        socket,
                        clients,
                        &format!("/gsrd/state/object/{}/gain", idx),
                        gain,
                    );
                }
            }
            ...
            if let Ok(idx) = idx_str.parse::<usize>() {
                ...
                if let Some(muted) = muted_opt {
                    ...
                    broadcast_int(
                        socket,
                        clients,
                        &format!("/gsrd/state/object/{}/mute", idx),
                        if muted { 1 } else { 0 },
                    );
    ...
    // ── Speaker collection edit: /gsrd/control/speakers/{add|remove|move} ──
    if addr == "/gsrd/control/speakers/add" {
    ...
    if addr == "/gsrd/control/speakers/remove" {
        pending_speakers.clear();
    ...
    if addr == "/gsrd/control/speakers/move" {
        pending_speakers.clear();
    ...
    // ── Speaker staging: /gsrd/control/speaker/{idx}/{az|el|distance|spatialize|name} ──
    if let Some(rest) = addr.strip_prefix("/gsrd/control/speaker/") {
                ...
                if parts[1] == "mute" {
                    ...
                    if let Some(muted) = muted_opt {
                        ...
                        broadcast_int(
                            socket,
                            clients,
                            &format!("/gsrd/state/speaker/{}/mute", idx),
                            if muted { 1 } else { 0 },
                        );
                    ...
                    match parts[1] {
                        ...
                        "gain" => {
                            ...
                            broadcast_float(
                                socket,
                                clients,
                                &format!("/gsrd/state/speaker/{}/gain", idx),
                                gain,
                            );
                        }
                        "delay" => {
                            ...
                            broadcast_float(
                                socket,
                                clients,
                                &format!("/gsrd/state/speaker/{}/delay", idx),
                                delay_ms,
                            );
    ...
    if addr == "/gsrd/control/speakers/apply" {
    ...
    if addr == "/gsrd/control/speakers/reset" {
        pending_speakers.clear();
        return;
    }
}
...
fn trigger_layout_recompute(
    ...
    #[cfg(not(feature = "sparta"))]
    {
        let _ = control;
        log::warn!("OSC apply: VBAP recompute requires a build with the 'sparta' feature");
                    ...
                    Ok(new_topology) => {
                        ...
                        for (idx, speaker) in rebuild_plan_for_thread.layout.speakers.iter().enumerate() {
                            ...
                            broadcast_int(
                                &socket_clone,
                                &clients_clone,
                                &format!("/gsrd/state/speaker/{}/spatialize", idx),
                                if speaker.spatialize { 1 } else { 0 },
                            );
                            broadcast_string(
                                &socket_clone,
                                &clients_clone,
                                &format!("/gsrd/state/speaker/{}/name", idx),
                                &speaker.name,
                            );
                        }
...
fn build_live_state_bundle(control: &Arc<RendererControl>) -> Vec<u8> {
    ...
    let mut messages = vec![
        ...
        OscPacket::Message(OscMessage {
            addr: "/gsrd/state/loudness".to_string(),
            args: vec![OscType::Int(if live.use_loudness { 1 } else { 0 })],
        }),
        OscPacket::Message(OscMessage {
            addr: "/gsrd/state/room_ratio".to_string(),
        ...
        OscPacket::Message(OscMessage {
            addr: "/gsrd/state/room_ratio_rear".to_string(),
            args: vec![OscType::Float(live.room_ratio_rear)],
        }),
        OscPacket::Message(OscMessage {
            addr: "/gsrd/state/room_ratio_center_blend".to_string(),
            args: vec![OscType::Float(live.room_ratio_center_blend)],
        }),
        OscPacket::Message(OscMessage {
            addr: "/gsrd/state/distance_diffuse/enabled".to_string(),
            args: vec![OscType::Int(if live.use_distance_diffuse { 1 } else { 0 })],
        }),
        OscPacket::Message(OscMessage {
            addr: "/gsrd/state/distance_diffuse/threshold".to_string(),
            args: vec![OscType::Float(live.distance_diffuse_threshold)],
        }),
        OscPacket::Message(OscMessage {
            addr: "/gsrd/state/distance_diffuse/curve".to_string(),
            args: vec![OscType::Float(live.distance_diffuse_curve)],
        }),
        OscPacket::Message(OscMessage {
            addr: "/gsrd/state/config/saved".to_string(),
            args: vec![OscType::Int(
                if control
                    .config_dirty
                    .load(std::sync::atomic::Ordering::Relaxed)
        ...
        OscPacket::Message(OscMessage {
            addr: "/gsrd/state/layout/radius_m".to_string(),
            args: vec![OscType::Float(radius_m)],
        }),
        OscPacket::Message(OscMessage {
            addr: "/gsrd/state/adaptive_resampling".to_string(),
    ...
    for (&idx, obj) in &live.objects {
        if obj.gain != 1.0 {
            all_messages.push(OscPacket::Message(OscMessage {
                addr: format!("/gsrd/state/object/{}/gain", idx),
                args: vec![OscType::Float(obj.gain)],
            }));
        }
        if obj.muted {
            all_messages.push(OscPacket::Message(OscMessage {
                addr: format!("/gsrd/state/object/{}/mute", idx),
    ...
    for (&idx, sp) in &live.speakers {
        if sp.gain != 1.0 {
            all_messages.push(OscPacket::Message(OscMessage {
                addr: format!("/gsrd/state/speaker/{}/gain", idx),
                args: vec![OscType::Float(sp.gain)],
            }));
        }
        if sp.delay_ms != 0.0 {
            all_messages.push(OscPacket::Message(OscMessage {
                addr: format!("/gsrd/state/speaker/{}/delay", idx),
                args: vec![OscType::Float(sp.delay_ms)],
            }));
        }
        if sp.muted {
            all_messages.push(OscPacket::Message(OscMessage {
                addr: format!("/gsrd/state/speaker/{}/mute", idx),
                args: vec![OscType::Int(1)],
            }));
        }
    }
    ...
    // Speaker names from current layout (always sent so UI can resync labels).
    {
        ...
        for (idx, speaker) in layout.speakers.iter().enumerate() {
            all_messages.push(OscPacket::Message(OscMessage {
                addr: format!("/gsrd/state/speaker/{}/name", idx),
    ...
    // Dialog normalization gain is always sent (1.0 = no correction applied).
    {
        ...
        all_messages.push(OscPacket::Message(OscMessage {
            addr: "/gsrd/state/loudness/gain".to_string(),
    ...
    if let Some(dl) = live.dialogue_level {
        all_messages.push(OscPacket::Message(OscMessage {
            addr: "/gsrd/state/loudness/source".to_string(),
...
fn set_dirty(
    ...
    control.mark_dirty();
    broadcast_int(socket, clients, "/gsrd/state/config/saved", 0);
}
...
/// Broadcasts `/gsrd/state/config/saved 1` on success; logs error and stays dirty on failure.
fn save_live_config(
    control: &Arc<RendererControl>,
    socket: &UdpSocket,
    clients: &Mutex<HashMap<SocketAddr, Option<Instant>>>,
    ...
    match config.save(&path) {
        Ok(()) => {
            control.mark_clean();
            broadcast_int(socket, clients, "/gsrd/state/config/saved", 1);
...
fn send_raw(
    ...
    let now = Instant::now();
    for (addr, last_seen) in clients_locked.iter() {
        let alive = match last_seen {
            None => true,
            Some(t) => now.duration_since(*t) < CLIENT_TIMEOUT,
        };
        if alive {
...
/// Resolve the client address for a `/gsrd/register` message.
