...
fn merge_render_config(cfg: &renderer::config::RenderConfig, args: &mut RenderArgs) {
    ...
    if args.sink.is_none() {
        if let Some(ref s) = cfg.sink {
            args.sink = Some(s.clone());
        }
    }
    #[cfg(all(target_os = "windows", feature = "asio"))]
    if args.asio_device_name.is_none() {
        if let Some(ref s) = cfg.asio_device_name {
            args.asio_device_name = Some(s.clone());
        }
    }
    ...
    // osc_metering
    if !args.osc_metering && !args.no_osc_metering {
        args.osc_metering = cfg.osc_metering.unwrap_or(false);
...
fn init_render_handler(
    ...
    // Set PipeWire sink target and buffer config (Linux only)
    #[cfg(all(target_os = "linux", feature = "pipewire"))]
    {
    ...
    // Set ASIO device name if specified (Windows only)
    #[cfg(all(target_os = "windows", feature = "asio"))]
    {
        handler.runtime.asio_device_name = args.asio_device_name.clone();
    }
    ...
    if args.enable_vbap {
        ...
        let renderer = if let Some(ref vbap_table_path) = args.vbap_table {
            ...
            // Runtime VBAP generation (requires 'sparta' feature)
            #[cfg(feature = "sparta")]
            {
            ...
            {
                anyhow::bail!(
                    "VBAP table generation not available (built without 'sparta' feature).\n\
                     Please provide a pre-generated VBAP table using --vbap-table <file>.\n\
                     Generate tables on a system with SPARTA support using:\n  \
                     gsrd generate-vbap --speaker-layout <layout.yaml> --output <table.vbap>"
                );
            }
        };
    ...
    // Initialize audio meter if explicitly requested (requires VBAP + OSC)
    if args.osc_metering {
        match (&handler.spatial_renderer, &handler.telemetry.osc_sender) {
            (Some(renderer), Some(_)) => {
                ...
                log::info!("OSC metering enabled ({} speakers, 20 Hz)", num_speakers);
            }
            (None, _) => log::warn!("--osc-metering requires --enable-vbap, metering disabled"),
            (_, None) => log::warn!("--osc-metering requires --osc, metering disabled"),
        }
    }
...
fn handle_flush_request(handler: &DecodeHandler) {
    // Seek/decoder reset: discard stale buffered audio so post-seek
    // audio starts cleanly instead of playing out pre-seek frames first.
    log::info!("Received flush request after decoder reset");
    if let Some(ref writer) = handler.output.audio_writer {
