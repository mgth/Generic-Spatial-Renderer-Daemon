...
#[command(
    ...
    long_about = None,
    after_help = "If no command is given, gsrd runs the default render flow.",
)]
pub struct Cli {
    /// Path to config file (default: ~/.config/gsrd/config.yaml)
...
pub struct RenderArgs {
    ...
    /// OSC registration listener port. Clients register by sending /gsrd/register
    /// to this port and receive the speaker config + all subsequent broadcasts.
    #[arg(long, value_name = "PORT", default_value_t = 9000)]
    pub osc_rx_port: u16,

    /// [LINUX ONLY] PipeWire sink target name (e.g., "gsrd_router")
    ...
    /// (By default, gsrd drains FIFOs to minimize latency for real-time streams)
    #[arg(long)]
    pub no_drain_pipe: bool,

    /// [WINDOWS ONLY] ASIO device name to use for output.
    /// Use 'gsrd list-asio-devices' to see available devices.
    #[cfg(all(target_os = "windows", feature = "asio"))]
