...
/// for the first client connection (blocking ConnectNamedPipe is fine here
/// because this happens once at startup before any audio processing begins).
#[cfg(windows)]
fn create_new_pipe_server_overlapped(
    ...
    // ConnectNamedPipe with NULL OVERLAPPED blocks until a client connects even
    // on an overlapped-mode handle — that's intentional (one-time startup cost).
    let connect_result = unsafe { ConnectNamedPipe(handle, None) };
    if connect_result.is_err() {
        let e = connect_result.err().unwrap();
        let err_code = e.code().0 as i32;
        // ERROR_PIPE_CONNECTED (535): client already connected — OK.
        if err_code != 535 {
            return Err(anyhow::anyhow!("Failed to connect named pipe: {}", e));
        }
    }
        ...
        F: FnMut(&[u8]) -> Result<bool>,
    {
        ...
        #[cfg(unix)]
        {
            ...
            loop {
                ...
                // Signal fd became readable (shutdown or reload).
